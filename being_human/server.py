...
def run(module, args, timeout=180):
    """
    Invoke a sibling module as `python -m being_human.<module>`.

    Addressing them as modules rather than file paths is what lets this work
    both from a source checkout and from an installed wheel, where there is no
    scripts/ directory to point at.
    """
    try:
        proc = subprocess.run(
            [sys.executable, "-m", "being_human." + module] + args,
...
def rules_block():
    ...
    output.
    """
    code, out, _ = run("export", ["--in", voice_dir(), "--target", "system"])
...
def tool_voice_score(args):
    ...
    code, out, err = run("slopscore", [path,
                          "--in", voice_dir(), "--json"])
...
def tool_voice_learn(args):
    ...
    directory = voice_dir()
    harvest = ["--source", source, "--out", directory]
    if path:
    ...
    code, out, err = run("harvest", harvest)
    if code != 0:
    ...
    code2, out2, err2 = run("analyze", analyze)
    if code2 != 0:
...
def tool_voice_export(args):
    code, out, err = run("export", ["--in", voice_dir(), "--target", args["target"]])
    if code != 0:
